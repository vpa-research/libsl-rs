...
use crate::{AnnotationId, DeclId, ExprId, LibSlNode, PredId, StmtId, TyExprId, WithLibSl, ast};
...
impl Serialize for LibSlNode<'_, AnnotationId> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        self.map(|&annotation_id| &self.libsl().annotations[annotation_id])
            .serialize(serializer)
    }
}
...
impl Serialize for LibSlNode<'_, ast::Annotation> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        let mut state = serializer.serialize_struct("Annotation", 2)?;
        state.serialize_field("name", &self.map(|decl| &decl.name))?;
        state.serialize_field("args", &self.map(|decl| &decl.args))?;
